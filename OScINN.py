...
class OScINN1D:
    ...
    def eval_inverse(self, data):
        ...
        with torch.no_grad():
            # Debug: Print the shape of c before processing
            print("Initial z shape:", z.shape)
            print("Initial c shape:", c.shape)

            # Adjust dimensions of c
            c = c.unsqueeze(dim=1) if is_batch else tile(c.unsqueeze(dim=0).unsqueeze(dim=0), 0, 2)
            print("Adjusted c shape before cond_net:", c.shape)

            # Process c through cond_net
            c = self.cond_net(c)
            print("Processed c shape after cond_net:", c.shape)

            c = [c.squeeze() for i in range(self.num_of_blocks)]
            print("Prepared c shape for each block:", len(c), c[0].shape)

            # Adjust dimensions of z
            ...
            print("Generated x_hat shape:", x_hat.shape)
